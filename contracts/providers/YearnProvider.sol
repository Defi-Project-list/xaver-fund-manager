...
contract YearnProvider is IProvider {
    ...
    modifier onlyControllerOrDao() {
        require(
            msg.sender == controller ||
                msg.sender == CompoundController(controller).dao(),
        ...
        onlySmartYieldOrController
    {
        ...
        IERC20(uToken).safeTransferFrom(
            from_,
            address(this),
            underlyingAmount_
        );
    ...
    function _depositProviderInternal(
    ...
    function _withdrawProviderInternal(
        uint256 underlyingAmount_,
        uint256 takeFees_
    ) internal {
        // underlyingFees += takeFees_;
    ...
    function balance() external view returns (uint256) {
        ...
        return (balanceShares * price) / EXP_SCALE;
    }
    ...
    function underlyingBalance() external virtual override returns (uint256) {
        ...
        return
            yTokenBalance.mul(exchangeRateCurrent()).div(EXP_SCALE).sub(
                underlyingFees
            );
    }
